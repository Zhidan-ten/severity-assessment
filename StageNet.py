...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=4):
        self.inplanes = 64
...
class MYmodel(nn.Module):
    def __init__(self,model, class_num=4 ):
        super().__init__()


        self.channel_all = 9216
        self.model = res2net50_26w_4s()
        ...
        self.linear = nn.Linear(self.channel_all, class_num)
    def forward(self, x1,y1,x2,y2,m1,m2,m3,m4):
        fcx1 = self.model(x1)
        fcy1 = self.model(y1)
        fcx2 = self.model(x2)
        fcy2 = self.model(y2)
        feature1 = torch.cat([fcx1,fcx2,fcy1,fcy2],1)
        m1.unsqueeze_(1)
        m2.unsqueeze_(1)
        m3.unsqueeze_(1)
        m4.unsqueeze_(1)
        four1 = torch.cat([m1, m2], 1)
        four2 = torch.cat([four1, m3], 1)
        four3 = torch.cat([four2, m4], 1)
        four = self.classifier(four3)
        feature5 = torch.cat([feature1, four], 1)
        out = self.linear(feature5)
        return out



def mymodel():
    return MYmodel(res2net50_26w_4s())
